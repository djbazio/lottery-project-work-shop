...
class BranchController extends Controller
{
    public function index()
    {
        ...
        return view('auth.admin.manage_member.manage_branch', compact('branches'));
    }

    public function store(Request $request)
    {
        if ($request->id != "") {
            $customers = Branch::find($request->id);
            $request->validate(
                [
                    "username" => $customers->username != $request->username ? "required|min:6|max:12|unique:customers|unique:users|unique:branches|unique:sellers" : "required|min:6|max:12|unique:users|unique:customers|unique:sellers",
            ...
            $user = Branch::updateOrCreate(['id' => $request->id], [
    ...
    public function delete_all_branch(Request $request)
    {
        $data = $request->pass;
        for ($i = 0; $i < count($data); $i++) {
            Branch::find($data[$i]['id'])->delete();
        }
