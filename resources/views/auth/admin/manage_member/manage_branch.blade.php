...
@section('content')
    ...
    <script>
        ...
        function createPost() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let _url = "{{ route('admin.data.manage.branch.store') }}";
        ...
        function editPost(event) {
            ...
            let _url = "/admin/api/getbranchData/" + id;
            let _token = $('meta[name="csrf-token"]').attr('content');
            clearTextModalError();
        ...
        function deletePost(event) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var id = $(event).data("id");
                    let _url = "/admin/data/delete/branch/" + id;
                    let _token = $('meta[name="csrf-token"]').attr('content');
    ...
    <script>
        ...
        function select_delete() {
            var arr = [];
            var _url = "{{ route('admin.data.delete.all_branch') }}";
            let _token = $('meta[name="csrf-token"]').attr('content');
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">จัดการข้อมูลสาขาย่อย</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('admin.index') }}">หน้าแรก</a></li>
                        <li class="breadcrumb-item active">จัดการข้อมูลสาขาย่อย</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
    ...
    <section class="content">
        <div class="container-fluid">
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">รายชื่อสาขาย่อย</h3>
                            ...
                            <table class="table table-head-fixed text-nowrap" id="table_crud">
                                ...
                                <tbody>
                                    @foreach ($branches as $branch)
                                        <tr id="row_{{ $branch->id }}" align="center">
                                            <th id="td_choese" class="align-middle" hidden>
                                                <div align="center">
                                                    <input type="checkbox" class="form-check" name="select"
                                                        id="select_input" value="{{ $branch->id }}">
                                                </div>
                                            </th>
                                            <td class="align-middle">{{ $branch->id }}</td>
                                            <td class="align-middle">{{ $branch->fname }} {{ $branch->lname }}</td>
                                            <td class="align-middle">{{ $branch->tel }}</td>
                                            <td class="align-middle">{{ $branch->address }}</td>
                                            <td class="align-middle">{{ $branch->username }}</td>
                                            <td class="align-middle" align="center">
                                                <a href="javascript:void(0)" class="btn btn-warning"
                                                    data-id="{{ $branch->id }}" onclick="editPost(event.target)"
                                                    id='btn_edit'>แก้ไข</a>
                                                <a href="javascript:void(0)" class="btn btn-danger"
                                                    data-id="{{ $branch->id }}" onclick="deletePost(event.target)"
                                                    id='btn_delete'>ลบ</a>
                                            </td>
                                        </tr>
                                    @endforeach
            ...
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title">เพิ่มข้อมูลสาขาย่อย</h3>
                </div>
                <!-- /.card-header -->
                <!-- form start -->
                <form>
