...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">ข้อมูลสลาก (สาขาย่อย)</h1>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ route('admin.index') }}">หน้าแรก</a></li>
                        <li class="breadcrumb-item active">ข้อมูลสลาก (สาขาย่อย)</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
                            ...
                            <table class="table table-head-fixed text-nowrap" id="table_crud">
                                <thead>
                                    <tr align="center">
                                        ...
                                        <th>สถานะ</th>
                                        <th>ราคา</th>
                                        <th>ฝากขายโดย (ผู้ขาย)</th>
                                        {{-- <th>อื่นๆ</th> --}}
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach ($consignments as $consignment)
                                        <tr id="row_{{ $consignment->id }}" align="center">
                                            <th id="td_choese" class="align-middle" hidden>
                                                <div align="center">
                                                    <input type="checkbox" class="form-check" name="select"
                                                        id="select_input" value="{{ $consignment->id }}">
                                                </div>
                                            </th>
                                            <td class="align-middle">{{ $consignment->id }}</td>
                                            <td class="align-middle">{{ $consignment->lottery->no }}</td>
                                            <td class="align-middle">{{ $consignment->lottery->set }}</td>
                                            <td class="align-middle">{{ $consignment->lottery->num }}</td>
                                            <td class="align-middle">{{ $consignment->lottery->volume }}</td>
                                            <td class="align-middle">
                                                @if ($consignment->lottery->status == '0')
                                                    เปิดขาย
                                                @else
                                                    ไม่เปิดขาย
                                                @endif
                                            </td>
                                            <td class="align-middle">{{ $consignment->lottery->price }}</td>
                                            <td class="align-middle">
                                                @if ($consignment->seller != null)
                                                    {{ $consignment->seller->fname }}
                                                    {{ $consignment->seller->lname }}
                                                @else
                                                    ไม่มีข้อมูล
                                                @endif
                                            </td>
